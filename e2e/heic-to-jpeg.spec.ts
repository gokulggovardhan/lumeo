...
function maliciousHeifFixtures(): { itemFlood: Buffer; auxCycle: Buffer; longReferenceChain: Buffer } {
  ...
  const chainEntries = Array.from({ length: 1001 }, (_, index) => ref("dimg", index + 1, index + 2));
  const longReferenceChain = Buffer.concat([
    heicFtyp(),
    bmffBox("meta", fullBox,
      bmffBox("pitm", fullBox, be16(1)),
      bmffBox("iref", fullBox, ...chainEntries)),
  ]);

  return { itemFlood, auxCycle, longReferenceChain };
}
...
test("malicious HEIF structures fail per-file before poisoning the next valid conversion", async ({ page }) => {
  ...
  const { itemFlood, auxCycle, longReferenceChain } = maliciousHeifFixtures();

  await page.goto("/heic-to-jpeg");
  await page.locator('input[type="file"]').setInputFiles([
    { name: "ITEM-FLOOD.HEIC", mimeType: "image/heic", buffer: itemFlood },
    { name: "AUX-CYCLE.HEIC", mimeType: "image/heic", buffer: auxCycle },
    { name: "LONG-IREF-CHAIN.HEIC", mimeType: "image/heic", buffer: longReferenceChain },
