...
| **Rate limiting** | Historical finding from this July 2026 audit: no dedicated rate-limiting/throttling middleware was found for admin login, the contact form, or public API routes. The Word↔PDF storage retry helpers cited by the original audit were retired with the server conversion backend in September 2026; that retirement does not by itself resolve the broader rate-limiting recommendation. |
| **Word↔PDF conversion architecture** | Historical note: this July 2026 audit reviewed a standalone LibreOffice conversion service. That service and its server API/storage path were retired in September 2026 after browser-only Word↔PDF production certification. Current conversion architecture is covered by the maintained browser conversion/runtime tests rather than this historical snapshot. |
| **Blob lifecycle / "nothing is stored"** | Homepage explicitly claims "Each file is processed for the task at hand, then cleared — no drafts, no cached copies." Consistent with the client-side pdf-lib processing observed in the live Merge PDF test. Historical note: the server-side Word↔PDF storage path mentioned by the original July audit was retired in September 2026 after browser-only production certification. |
| **Supabase RLS** | Partially confirmed live: an unauthenticated request using the app's own public/anon key (`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` from `.env.local`) against `GET /rest/v1/analytics_events` returned `42501 permission denied for table analytics_events` — RLS is actively enforced against the anon role for raw analytics data, matching the design intent confirmed elsewhere in this repo ("admin analytics reads use the aggregate RPC instead of direct event table reads"). Full RLS policy review (every table, every role) was not done — this is one confirmed data point, not a complete audit, and no service-role key was available to test what an authenticated admin *should* be able to see. |
