...
-- back the ENTIRE migration -- including the table itself -- leaving the
-- admin inbox querying a table that was never actually created. This
-- migration is idempotent (safe to run whether 014 fully, partially, or
-- never applied) and moves realtime into its own transaction that can never
...
-- Realtime enablement in its own transaction, tolerant of every way this can
-- already be true or unavailable: publication missing (realtime disabled for
-- this project), table already a member (re-running this migration), or any
-- other error -- none of these should ever be able to undo the table above.
do $$
begin
