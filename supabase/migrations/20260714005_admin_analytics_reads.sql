...
begin
  if to_regclass('public.analytics_events') is null then
    raise exception 'Missing required table public.analytics_events. Run prior analytics migrations before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regclass('public.daily_tool_metrics') is null then
    raise exception 'Missing required table public.daily_tool_metrics. Run prior analytics migrations before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regclass('public.pdf_tools') is null then
    raise exception 'Missing required table public.pdf_tools. Run prior catalog migrations before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regclass('public.site_settings') is null then
    raise exception 'Missing required table public.site_settings. Run Control Center migrations before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regclass('public.admin_members') is null then
    raise exception 'Missing required table public.admin_members. Run admin authentication migration before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regprocedure('public.current_admin_role()') is null then
    raise exception 'Missing required function public.current_admin_role(). Run Control Center foundation migration before 20260714005_admin_analytics_reads.sql.';
  end if;

  if to_regprocedure('public.is_active_admin()') is null then
    raise exception 'Missing required function public.is_active_admin(). Run Control Center foundation migration before 20260714005_admin_analytics_reads.sql.';
  end if;
end;
$$;
