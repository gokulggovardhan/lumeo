...
export default function PdfToWordTool() {
  ...
  return (
    <section className="l2-workspace-deep grid min-w-0 gap-4 pb-28 lg:pb-6">
      ...
      <L2WorkspaceHeader
        title="PDF to Word"
        description={`PDF · ${formatFileSize(selected.file.size)}`}
      />
      ...
      <L2WorkspaceGrid
        main={
          <L2WorkspacePanel variant="flat">
            <L2FileCard
              ...
              meta={`PDF · ${formatFileSize(selected.file.size)}`}
              icon={
                <FileText
                  aria-hidden="true"
            ...
            {stage === "cancelled" ? (
              ...
              >
                ...
                <p className="mt-1 text-xs leading-5 text-[var(--text-subtle)]">
                  Your selected PDF is still ready if you want to try again.
                </p>
              </div>
            ) : null}
              ...
              <div className="aura-success-reveal mt-3 flex min-w-0 items-start gap-3 rounded-xl border border-[rgb(var(--emerald-rgb)/0.36)] bg-[var(--surface-success)] p-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-[var(--text-primary)]/56">
                    DOCX · {formatFileSize(result.blob.size)}
                  </p>
                </div>
              </div>
            ) : null}
        ...
        inspector={
          <L2WorkspaceInspector
            ...
            description="Lumeo reconstructs an editable Word document while preserving the original layout as closely as possible."
          >
            <p className="mt-3 text-xs leading-5 text-[var(--text-subtle)]">
              Digital text stays editable where it can be reconstructed
              reliably. Page dimensions, text positioning, font sizing,
              bold/italic styling, images, and page order are preserved where
              supported. Image-only pages use a layout-preserving page image
              unless OCR is available.
            </p>
            <p className="mt-3 text-xs leading-5 text-[var(--text-subtle)]">
              Processed locally in your browser. Your document is not uploaded
              for conversion.
            </p>
          </L2WorkspaceInspector>
        }
      ...
      <ToolActionBar>
        {result ? (
          ...
          <button
            ...
            aria-label="Cancel PDF to Word conversion"
