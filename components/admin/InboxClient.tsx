...
import { formatAdminRelativeTime } from "@/lib/admin/relative-time";
...
export function InboxClient({
  ...
  // The first server render and first client render must use the exact same
  // reference clock. Only advance it after hydration has completed so relative
  // labels cannot cross a minute boundary and trigger a text mismatch.
  useEffect(() => {
                  ...
                  <li key={item.id}>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="shrink-0 text-xs text-[var(--lumeo-paper-500)]">{formatAdminRelativeTime(item.created_at, relativeNowMs)}</span>
                      </div>
